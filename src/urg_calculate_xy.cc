...
vector<double> urg_calculate_xy(const char* device)
{
    urg_t urg;
    long *data;
    long max_distance;
    ...
    for (i = 0; i < n; ++i) {
        ...
        arr[i] = distance * cos(radian) / M_TO_MM;
        arr[1081+i] = distance * sin(radian) / M_TO_MM;
        //printf("%ld, %ld\n", arr[i], arr[1081+i]);
    }
    //printf("\n");
